...
그냥 박스만 그리는 것을 넘어, 실제 필요로 하는 로직을 간단하게 구현해 봅니다. 파이썬의 조건문(If)과 리스트를 활용합니다.

+ 아이디어 A: 특정 객체(사람/차량)만 탐지하기

YOLO는 80가지 객체를 탐지하지만, 국방에서는 사람(class 0), 차량(class 2)만 중요할 수 있습니다.

model('image.jpg', classes=[0, 2]) 처럼 옵션을 주어 필터링하는 법을 배웁니다.

+ 아이디어 B: 침입 감지 알람 (객체 수 카운팅)

"경계 구역 내에 사람이 3명 이상 포착되면 경보를 울린다."

추론 결과 데이터 구조를 분해하여, 탐지된 '사람'의 수를 세고 조건문을 처리합니다.
